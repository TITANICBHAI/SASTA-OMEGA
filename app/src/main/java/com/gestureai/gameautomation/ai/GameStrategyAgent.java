...
public class GameStrategyAgent {
    ...
    private float epsilon = 0.3f; // Exploration rate
    ...
    public GameStrategyAgent(Context context) {
    ...
    public enum GameType { BATTLE_ROYALE, MOBA, FPS, STRATEGY, ARCADE, RACING, PUZZLE }
    ...
    public class BattleRoyaleStrategy extends BaseStrategy {
        ...
        private INDArray createBRStateArray(UniversalGameState state, float zoneRisk,
            ...
            // Copy base state
            System.arraycopy(gameStateToArray(state).toFloatVector(), 0, brState, 0, STATE_SIZE);
        ...
        private GameAction createBattleRoyaleAction(int actionIndex, UniversalGameState state,
            ...
            switch (actionType) {
                case "ROTATE_TO_ZONE":
                    ...
                    break;
                case "ENGAGE_ENEMY":
                    x = state.nearestObjectX;
                    y = state.nearestObjectY;
                    break;
                case "SEEK_COVER":
    ...
    public class MOBAStrategy extends BaseStrategy {
        ...
        private INDArray createMOBAStateArray(UniversalGameState state, float farmPriority,
            ...
            System.arraycopy(gameStateToArray(state).toFloatVector(), 0, mobaState, 0, STATE_SIZE);
    ...
    public class FPSStrategy extends BaseStrategy {
        ...
        private INDArray createFPSStateArray(UniversalGameState state, float weaponEffectiveness, 
            ...
            // Copy base state
            System.arraycopy(gameStateToArray(state).toFloatVector(), 0, fpsState, 0, STATE_SIZE);
    ...
    // Enhanced strategy selection and coordination
    public GameAction analyzeGameContext(GameType gameType, UniversalGameState state) {
        BaseStrategy strategy = getStrategyForGameType(gameType);

        if (strategy != null) {
            GameAction strategicAction = strategy.analyzeGameContext(state);

            // Combine with base DQN decision
            GameAction dqnAction = selectOptimalAction(state);
    ...
    private INDArray gameStateToArray(UniversalGameState state) {
        ...
        stateData[2] = state.screenWidth / 1080f; // Screen dimensions
        stateData[3] = state.screenHeight / 1920f;
    ...
    private int calculateActionX(String actionType, UniversalGameState state) {
        switch (actionType) {
            ...
            case "LONG_PRESS":
                ...
                return state.nearestObjectX > 0 ? state.nearestObjectX : state.playerX;
            default:
